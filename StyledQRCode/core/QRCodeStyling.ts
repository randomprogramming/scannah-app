...
import { CustomCanvas } from "./QRCanvas";
...
export default class QRCodeStyling {
  ...
  append(container?: HTMLElement): void {
    ...
    if (this._canvas && !this._options.nodeCanvas) {
      let cnv = <CustomCanvas>this._canvas.getCanvas();
      ...
      this._drawingPromise
        .then(() => {
          ...
          if (this._options.nodeCanvas && buffer) {
            data =
              this._canvas.getCanvas().toBuffer?.(`${extension}`) ??
              Buffer.from([]);
          } else {
